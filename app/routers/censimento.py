...
async def create_censimento(
    ...
    #db_item = EmailDataModel(**item.dict(), typo="censimento", user_id=user_id)
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
...
async def list_censimento_no_auth(city: Optional[str] = None, db: Session = Depends(get_db)):
    """
    Endpoint di test senza autenticazione per censimento
    """
    try:
        records = db.query(EmailDataModel).filter(EmailDataModel.typo == "censimento")
        if city:
        ...
        logger.info(f"Recuperati {len(records)} record censimento (senza autenticazione), "
        ...
        logger.error(f"Errore nel recupero censimento senza auth: {str(e)}")
        raise HTTPException(
